import { identityComponent, svgTags, resolveObservableState, isFunction, resolveEventName, isBoolean, isString, isSomeVNode, isUndefined, isPlainObject, omit, isPromise, isArray, resolveNextNodes, normalizeElementProps, noop } from '@setsuna/share';
...
const records$1 = new Map();

function registryRecord(id, renderEffect) {
  let record = records$1.get(id);
  if (!record) {
    records$1.set(id, (record = { deps: new Set() }));
  }
...
function invokeReload(id, App) {
  const appRecord = records$1.get(id);
...
export { Await, Fragment, Teleport, _jsx, _node_flag, callWithErrorHandler, defineElement, getCurrentInstance, hydrateChildren, hydrateProps, ignoreElement, isVNode, mountChildren, normalizeChildren, patch$1 as patch, patchChildren, patchProps, setCurrentInstance, unmount };
